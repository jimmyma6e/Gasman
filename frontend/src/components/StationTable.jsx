...
export default function StationTable({ stations, cheapestPrices, favourites, onToggleFavourite, onOpenChart }) {
  ...
  const sorted = [...stations].sort((a, b) => {
    let av, bv;
    if (FUEL_TYPES.some((f) => f.key === sortCol)) {
      av = a[sortCol]?.price ?? Infinity;
      bv = b[sortCol]?.price ?? Infinity;
    } else if (sortCol === "name") {
      return sortDir * a.name.localeCompare(b.name);
    } else if (sortCol === "area") {
      return sortDir * (a._area ?? "").localeCompare(b._area ?? "");
    } else {
      av = a[sortCol]?.price ?? Infinity;
      bv = b[sortCol]?.price ?? Infinity;
    }
  ...
  function ColHeader({ col, label }) {
    ...
    return (
      <th
        className={`tbl-th tbl-sortable ${active ? "tbl-th-active" : ""}`}
        ...
        <thead>
          <tr>
            ...
            {FUEL_TYPES.map(({ key, label }) => (
              <ColHeader key={key} col={key} label={label} />
            ))}
              ...
              <tr key={s.station_id} className="tbl-row">
                <td className="tbl-td tbl-fav-col">
                  <button
                    ...
                    onClick={() => onToggleFavourite(s.station_id)}
                  >
                    {isFav ? "★" : "☆"}
                  </button>
                </td>
                ...
                <td className="tbl-td tbl-addr">
                  <a
                    className="tbl-addr-link"
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${s.address}, ${s._area}, BC`)}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {s.address}
